...
-- 2. Replace team_members self-referencing policies
...
-- 3. Replace teams policies that reference team_members
...
-- 4. Replace team_invites policies
...
-- 5. Replace team_shares policies
...
-- 6. Replace api_keys team policy
...
CREATE POLICY "Team editors can manage team keys" ON api_keys FOR ALL
  TO authenticated USING (current_user_has_team_role(team_id, ARRAY['owner', 'editor']))
  WITH CHECK (current_user_has_team_role(team_id, ARRAY['owner', 'editor']));

-- 7. Restore add_team_owner trigger (clean SECURITY DEFINER, no role switch needed)
